...
import { Shield, Award, Search, Home, Heart, GraduationCap, Users, Briefcase, ChevronRight, Clock, DollarSign, BadgeCheck, Phone, ArrowRight, Loader2 } from 'lucide-react';
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, index) => (
              <div key={index} className="relative group">
                <div className="bg-slate-50 rounded-2xl p-8 h-full border border-slate-100 hover:border-brand-marine/20 hover:shadow-xl transition-all duration-300">
                ...
                {index < steps.length - 1 && (
                  <div className="hidden lg:flex absolute top-1/2 -right-4 transform -translate-y-1/2 z-10">
                    <ChevronRight className="w-8 h-8 text-brand-marine/30" />
                  </div>
                )}
              </div>
            ))}
            ...
            {holisticPillars.map((pillar, index) => (
              ...
              >
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 ${
                  ...
                  pillar.color === 'blue' ? 'bg-brand-marine/10' :
                  'bg-emerald-100'
                }`}>
                  <pillar.icon className={`w-8 h-8 ${
                    pillar.color === 'rose' ? 'text-rose-600' :
                    pillar.color === 'blue' ? 'text-brand-marine' :
                    'text-emerald-600'
                  }`} />
                </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-8 p-4 md:p-8">
              ...
              <div className="flex-shrink-0">
                <Button
                  ...
                  className="bg-brand-marine hover:bg-brand-marine/90"
                >
                  Inquire
                </Button>
              </div>
